...
export default function Tutors() {
  ...
  const filteredTutors = tutors.filter(tutor => {
    ...
    // Check mode filter (online/visit_student/tutor_tuition)
    const matchesModes = selectedModes.length === 0 || 
      selectedModes.some(mode => {
        const tutorMode = tutor.teaching_mode?.toLowerCase() || '';
        if (mode === 'online') {
          return tutorMode.includes('online');
        } else if (mode === 'visit_student') {
          return tutorMode.includes('visit_student') || tutorMode.includes('home visit');
        } else if (mode === 'tutor_tuition') {
          return tutorMode.includes('tutor_tuition') || tutorMode.includes('home tuition') || tutorMode.includes('tutor place');
        }
        return false;
      });
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2 border rounded-md p-2">
                  {['online', 'visit_student', 'tutor_tuition'].map((mode) => (
                    <label key={mode} className="flex items-center space-x-2 text-sm">
                      ...
                      <span className="capitalize">
                        {mode === 'online' ? 'Online Mode' : 
                         mode === 'visit_student' ? 'Visit Student (Home Visit)' : 
                         mode === 'tutor_tuition' ? 'Tutor Tuition (Home Tuition at Tutor\'s Place)' : 
                         mode}
                      </span>
                    </label>
                  ))}
                  ...
                  <div className="flex flex-wrap gap-1">
                    {selectedModes.map((mode) => (
                      <Badge key={mode} variant="secondary" className="text-xs">
                        {mode === 'online' ? 'Online Mode' : 
                         mode === 'visit_student' ? 'Visit Student' : 
                         mode === 'tutor_tuition' ? 'Tutor Tuition' : 
                         mode}
                        <button
            ...
            <div className="text-center py-16">
              <div className="text-muted-foreground mb-4">
                ...
                <h3 className="text-xl font-semibold mb-2">
                  {searchTerm || selectedSubjects.length > 0 ? "No tutors match your search" : "No tutors available"}
                </h3>
                <p>
