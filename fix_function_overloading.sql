...
DROP FUNCTION IF EXISTS handle_requirement_tutor_match(UUID, UUID, TEXT, TEXT, DECIMAL);
DROP FUNCTION IF EXISTS handle_requirement_tutor_match(UUID, UUID, TEXT, TEXT, NUMERIC);
DROP FUNCTION IF EXISTS handle_requirement_tutor_match(UUID, UUID, TEXT, TEXT, NUMERIC, TEXT);

-- Create a uniquely named function to avoid conflicts
CREATE OR REPLACE FUNCTION handle_requirement_tutor_match_v2(
...
GRANT EXECUTE ON FUNCTION handle_requirement_tutor_match_v2(UUID, UUID, TEXT, TEXT, DECIMAL) TO authenticated;

-- Grant execute permission to anon users (if needed)
GRANT EXECUTE ON FUNCTION handle_requirement_tutor_match_v2(UUID, UUID, TEXT, TEXT, DECIMAL) TO anon;

-- Also create the original function name with exact signature match
...
AS $$
BEGIN
  -- Call the v2 function to avoid code duplication
  RETURN handle_requirement_tutor_match_v2(
    p_requirement_id,
    p_tutor_id,
    p_status,
    p_response_message,
